...
package atemis.MP3lib.decoder {
	 ...
	 */
	internal final class SynthesisFilter
	{
	  private var	 			 v1:Array;
	  private var			 	 v2:Array;
	  private var				 actual_v:Array;		// v1 or v2
	  private var 			 	 actual_write_pos:int;	// 0-15
	  private var				 samples:Array;			// 32 new subband samples
	   ...
	   */
	  public function SynthesisFilter(channelnumber:int, factor:Number, eq0:Array)
	  {  	 
		  if (d==null)
		  {
				d = load_d();
	  ...
	  public function setEQ(eq0:Array):void
	  {
		 this.eq = eq0;	 
		 if (eq==null)
		 {
	  ...
	  public function reset():void
	  {
		 ...
		 //   *--floatp = *--floatp2 = 0.0;
		 for (var p:int=0;p<512;p++) 
			 v1[p] = v2[p] = 0.0;
	
	     // initialize samples[]:
	     //for (floatp = samples + 32; floatp > samples; )
		 //  *--floatp = 0.0;
		 for (var p2:int=0;p2<32;p2++) 
			 samples[p2] = 0.0;
	   ...
	   * Inject Sample.
	   */
	  public function input_sample(sample:Number, subbandnumber:int):void
	  {	 	 		  
		  samples[subbandnumber] = eq[subbandnumber]*sample;
	  }
	
	  public function input_samples(s:Array):void
	  {
	  ...
	  private function compute_new_v():void
	  {
		...
		var	new_v0:Number, new_v1:Number, new_v2:Number, new_v3:Number, new_v4:Number, new_v5:Number, new_v6:Number, new_v7:Number, new_v8:Number, new_v9:Number;
		var new_v10:Number, new_v11:Number, new_v12:Number, new_v13:Number, new_v14:Number, new_v15:Number, new_v16:Number, new_v17:Number, new_v18:Number, new_v19:Number;
		var new_v20:Number, new_v21:Number, new_v22:Number, new_v23:Number, new_v24:Number, new_v25:Number, new_v26:Number, new_v27:Number, new_v28:Number, new_v29:Number;
		var new_v30:Number, new_v31:Number;
	...
	*/	
	  ...
	  private function compute_new_v_old():void
	  {
		// p is fully initialized from x1
		 ...
		 //float[] new_v = _new_v;
	  	...
	  	var new_v:Array = new Array(32); // new V[0-15] and V[33-48] of Figure 3-A.2 in ISO DIS 11172-3
		var p:Array = new Array(16);
		var pp:Array = new Array(16);
	...
	//	float[] pp = new float[16];
	  ...
	  private var _tmpOut:Array = new Array(32);
	  
	  
	  private function compute_pcm_samples0(buffer:Obuffer):void
	  {
		var vp:Array = actual_v;	
		//int inc = v_inc;
		var tmpOut:Array = _tmpOut;
		var dvp:int =0;
		
				// fat chance of having this loop unroll
		for(var i:int=0; i<32; i++)
		{
			var pcm_sample:Number;
			var dp:Array = d16[i];
	...
	*/	 
	  ...
	  private static const MY_PI:Number = 3.14159265358979323846;
	  private static const cos1_64:Number  = (1.0 / (2.0 * Math.cos(MY_PI        / 64.0)));
	  private static const cos3_64:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 3.0  / 64.0)));
	  private static const cos5_64:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 5.0  / 64.0)));
	  private static const cos7_64:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 7.0  / 64.0)));
	  private static const cos9_64:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 9.0  / 64.0)));
	  private static const cos11_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 11.0 / 64.0)));
	  private static const cos13_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 13.0 / 64.0)));
	  private static const cos15_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 15.0 / 64.0)));
	  private static const cos17_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 17.0 / 64.0)));
	  private static const cos19_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 19.0 / 64.0)));
	  private static const cos21_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 21.0 / 64.0)));
	  private static const cos23_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 23.0 / 64.0)));
	  private static const cos25_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 25.0 / 64.0)));
	  private static const cos27_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 27.0 / 64.0)));
	  private static const cos29_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 29.0 / 64.0)));
	  private static const cos31_64:Number = (1.0 / (2.0 * Math.cos(MY_PI * 31.0 / 64.0)));
	  private static const cos1_32:Number  = (1.0 / (2.0 * Math.cos(MY_PI        / 32.0)));
	  private static const cos3_32:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 3.0  / 32.0)));
	  private static const cos5_32:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 5.0  / 32.0)));
	  private static const cos7_32:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 7.0  / 32.0)));
	  private static const cos9_32:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 9.0  / 32.0)));
	  private static const cos11_32:Number = (1.0 / (2.0 * Math.cos(MY_PI * 11.0 / 32.0)));
	  private static const cos13_32:Number = (1.0 / (2.0 * Math.cos(MY_PI * 13.0 / 32.0)));
	  private static const cos15_32:Number = (1.0 / (2.0 * Math.cos(MY_PI * 15.0 / 32.0)));
	  private static const cos1_16:Number  = (1.0 / (2.0 * Math.cos(MY_PI        / 16.0)));
	  private static const cos3_16:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 3.0  / 16.0)));
	  private static const cos5_16:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 5.0  / 16.0)));
	  private static const cos7_16:Number  = (1.0 / (2.0 * Math.cos(MY_PI * 7.0  / 16.0)));
	  private static const cos1_8:Number   = (1.0 / (2.0 * Math.cos(MY_PI        / 8.0)));
	  private static const cos3_8:Number   = (1.0 / (2.0 * Math.cos(MY_PI * 3.0  / 8.0)));
	  private static const cos1_4:Number   = (1.0 / (2.0 * Math.cos(MY_PI / 4.0)));
	   ...
	   */
		 ...
		 */
		static private function splitArray(array:Array, blockSize:int):Array
		{
			var size:int = array.length / blockSize;
			var split:Array = new Array(size);
		 ...
		 */
		static private function subArray(array:Array, offs:int, len:int):Array
		{
			...
			var subarray:Array = new Array(len);
	  ...
	  private static const d_data:Array = [
